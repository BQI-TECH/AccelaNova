...
1. Clone the repo into your server as the user who the application will run as.
   `git clone git@github.com:BQI-TECH/AccelaNova.git Akili`

2. `cd Akili` and run `yarn setup`.
...
cd $HOME/Akili/frontend && yarn && yarn build && cd $HOME/Akili
...
cd $HOME/Akili/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/Akili/server && yarn
cd $HOME/Akili/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/Akili/server && npx prisma generate

echo "Booting up services."
truncate -s 0 /logs/server.log # Or any other log file location.
truncate -s 0 /logs/collector.log

cd $HOME/Akili/server
(NODE_ENV=production node index.js) &> /logs/server.log &

cd $HOME/Akili/collector

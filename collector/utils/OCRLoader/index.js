...
class OCRLoader {
  ...
  async ocrPDF(
    ...
    const canUseSharp = await pdfSharp.init();
    if (!canUseSharp) {
      // If sharp is unavailable we cannot perform image-based OCR on this PDF.
      // Return an empty document list so upstream callers can handle this gracefully.
      this.log(
        "Sharp is not available in this environment; skipping OCR for scanned PDF."
      );
      return [];
    }
...
class PDFSharp {
  ...
  async init() {
    ...
    try {
      ...
      // Some bundlers/libs export sharp as either default or named export.
      this.sharp = sharpModule.default || sharpModule;
      return true;
    } catch (error) {
      // If sharp cannot be loaded (e.g. native bindings missing or failed to install),
      // log a clear message and gracefully disable PDF OCR instead of crashing
      this.log(
        `Failed to load optional dependency "sharp". ` +
          `Scanned PDF OCR will be skipped. (${error.message})`
      );
      this.sharp = null;
      return false;
    }
  ...
  async pageToBuffer({ page }) {
    // Ensure sharp has been initialised; if init failed earlier this.sharp will be null.
    if (!this.sharp && !(await this.init())) {
      // sharp is unavailable on this runtime – skip OCR for this page
      this.log("Sharp module unavailable; skipping image extraction for page.");
      return null;
    }
    try {

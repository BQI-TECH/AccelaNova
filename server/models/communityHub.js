...
/**
 * An interface to the Accelanova Community Hub external API.
 */
const CommunityHub = {
    importPrefix: "allm-community-id",
    apiBase: process.env.NODE_ENV === "development" ?
        "http://127.0.0.1:5001/anythingllm-hub/us-central1/external/v1" : "https://hub.external.anythingllm.com/v1",
     ...
     * so we can unzip and import them to the Accelanova instance.
    ...
    /**
     * Apply an item to the Accelanova instance. Used for simple items like slash commands and system prompts.
    ...
    applyItem: async function(item, options = {}) {
        ...
        if (item.itemType === "system-prompt") {
            if (!(options && options.workspaceSlug))
        ...
        if (item.itemType === "slash-command") {
            ...
            await SlashCommandPresets.create((options && options.currentUser && options.currentUser.id) || null, {
    ...
    /**
     * Import a bundle item to the Accelanova instance by downloading the zip file and importing it.

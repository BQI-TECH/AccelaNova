...
function workspaceParsedFilesEndpoints(app) {
    ...
    app.get(
        "/workspace/:slug/parsed-files", [validatedRequest, flexUserRoleValid([ROLES.all]), validWorkspaceSlug],
    ...
    app.delete(
        "/workspace/:slug/delete-parsed-files", [validatedRequest, flexUserRoleValid([ROLES.all]), validWorkspaceSlug],
    ...
    app.post(
        ...
        async function(request, response) {
            try {
                ...
                if (!success || !documents || !documents[0]) {
                    return response.status(500).json({
                        success: false,
                ...
                const files = await Promise.all(
                    documents.map(async(doc) => {
                        ...
                        const safeBase = sanitizeFilename(
                            originalname.replace(/\.[^.]+$/, "") || "document"
                        );
                        const jsonBasename = `${safeBase}-${doc.id || Date.now()}.json`;
                        const jsonPath = path.join(directUploadsPath, jsonBasename);
                        fs.writeFileSync(
                            jsonPath,
                            JSON.stringify({...doc, pageContent }, null, 2),
                            "utf-8"
                        );
                        metadata.location = jsonBasename;
                ...
                await EventLogs.logEvent(
                    "document_uploaded_to_chat", {
                        ...
                        thread: (thread && thread.name) || null,
                    },
                    user && user.id
                );

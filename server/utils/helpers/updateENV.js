...
async function validDockerizedUrl(input = "") {
  ...
  try {
    ...
    if (isPortAvailableFromDocker)
      return "Port is not running a reachable service on loopback address from inside the Akili container. Please use host.docker.internal (for linux use 172.17.0.1), a real machine ip, or domain to connect to your service.";
  } catch (error) {
...
async function updateENV(newENVs = {}, force = false, userId = null) {
  ...
  const ENV_KEYS = Object.keys(newENVs).filter(
    (key) =>
      ...
      (typeof newENVs[key] !== "string" || !newENVs[key].includes("******")) // strip out answers where the value is all asterisks
  );
  const newValues = {};
...
function dumpENV() {
  ...
  // In Electron desktop builds, `server/.env` lives inside app.asar (read-only).
  // Persist env to a writable location instead.
  const isDesktop = String(process.env.DESKTOP_APP).toLowerCase() === "true";
  const envPath = isDesktop && process.env.STORAGE_DIR
    ? path.resolve(process.env.STORAGE_DIR, "desktop.env")
    : path.join(__dirname, "../../.env");

  try {
    if (isDesktop && process.env.STORAGE_DIR) {
      fs.mkdirSync(path.dirname(envPath), { recursive: true });
    }
    fs.writeFileSync(envPath, envResult, { encoding: "utf8", flag: "w" });
    return true;
  } catch (e) {
    // Never hard-fail the API call just because persistence failed.
    console.error(`Failed to write env file at ${envPath}:`, e?.message || e, e);
    return false;
  }
}

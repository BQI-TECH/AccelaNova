...
async function chatSync({
    ...
    const uuid = uuidv4();
    const chatMode = mode != null ? mode : "chat";
    ...
    const LLMConnector = getLLMProvider({
        provider: workspace && workspace.chatProvider,
        model: workspace && workspace.chatModel,
    });
    const VectorDb = getVectorDbClass();
    const messageLimit = workspace && workspace.openAiHistory || 20;
    ...
    if ((!hasVectorizedSpace || embeddingsCount === 0) && chatMode === "query") {
        const textResponse =
            (workspace && workspace.queryRefusalResponse) ||
    ...
    await new DocumentManager({
        ...
        .then((pinnedDocs) => {
            pinnedDocs.forEach((doc) => {
                ...
                sources.push({
                    text: pageContent.slice(0, 1 * 1000) +
    ...
    const parsedFilesSync = await WorkspaceParsedFiles.getContextFiles(
        workspace,
        thread || null,
        user || null
    );
    parsedFilesSync.forEach((doc) => {
        const { pageContent, title, ...metadata } = doc;
        const docTitle = title || metadata.location || "Uploaded document";
        const markdownContent =
            typeof pageContent === "string" && pageContent.trim() ?
            `## Document: ${docTitle}\n\n${pageContent.trim()}\n\n` :
            pageContent;
        contextTexts.push(markdownContent);
        sources.push({
            text: pageContent.slice(0, 1 * 1000) + "...continued on in source document...",
            ...metadata,
        });
    });
    ...
    const vectorSearchResults =
        ...
        await VectorDb.performSimilaritySearch({
            ...
            input: message,
            LLMConnector,
            similarityThreshold: workspace && workspace.similarityThreshold,
            topN: workspace && workspace.topN,
            filterIdentifiers: pinnedDocIdentifiers,
            rerank: workspace && workspace.vectorSearchMode === "rerank",
        }) : {
            contextTexts: [],
            sources: [],
            message: null,
    ...
    const filledSources = fillSourceWindow({
        nDocs: workspace && workspace.topN || 4,
    ...
    await LLMConnector.getChatCompletion(messages, {
        temperature: (workspace && workspace.openAiTemp) != null ? workspace.openAiTemp : LLMConnector.defaultTemp,
    });

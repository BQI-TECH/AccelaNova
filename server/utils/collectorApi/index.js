...
class CollectorApi {
  constructor() {
    ...
    // Prefer an explicit host override if provided; otherwise default to loopback
    // Using 0.0.0.0 as a destination is not routable on many systems (e.g., Windows)
    // so default to 127.0.0.1 for local, and allow overriding via COLLECTOR_HOST.
    const configuredHost = (process.env.COLLECTOR_HOST || "127.0.0.1").trim();
    const hostWithScheme = /^https?:\/\//.test(configuredHost)
      ? configuredHost
      : `http://${configuredHost}`;
    const port = process.env.COLLECTOR_PORT || 8888;
    this.endpoint = `${hostWithScheme}:${port}`;
  }

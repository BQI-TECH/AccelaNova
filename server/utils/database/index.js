...
async function checkForMigrations(model, db) {
    ...
    const toMigrate = [];
    for (const { colName, execCmd, doif }
        of model.migrations()) {
...
// of how Accelanova is used and how to improve this product!
// You can see all Telemetry events by ctrl+f `Telemetry.sendTelemetry` calls to verify this claim.
async function setupTelemetry() {
    if (process.env.DISABLE_TELEMETRY === "true") {
        console.log(
            `\x1b[31m[TELEMETRY DISABLED]\x1b[0m Telemetry is marked as disabled - no events will send. Telemetry helps Mintplex Labs Inc improve Accelanova.`
        );
        return true;
    }
    ...
    console.log(
        `\x1b[32m[TELEMETRY ENABLED]\x1b[0m Anonymous Telemetry enabled. Telemetry helps Mintplex Labs Inc improve Accelanova.`
    );

...
const LanceDb = {
         ...
         * In desktop builds we rely on STORAGE_DIR (user-writable). On Windows, always use path.resolve/join.
         * @returns {string}
         */
        uri: null,
        resolveUri: function() {
            ...
            try {
                ...
                // best-effort; connect() will surface a clearer error if this is unwritable
            }
            this.uri = lanceDir;
            return lanceDir;
        },
        ...
        connect: async function() {
            ...
            try {
                ...
                throw new Error(
                    `LanceDB connect failed at ${uri}. ${e?.message || String(e)}`
                );
            }
        },
            ...
            await reranker
                ...
                .then((rerankResults) => {
                    rerankResults.forEach((item) => {
                        ...
                        const score =
                            item.rerank_score ? item.rerank_score : this.distanceToSimilarity(item._distance);
  ...
  performSimilaritySearch: async function ({
    ...
    try {
      ...
      console.error("LanceDB::performSimilaritySearch", e?.message || e, e);
      return {
        contextTexts: [],
        sources: [],
        message: `Vector DB error (LanceDB). ${e?.message || String(e)}`,
      };
    }
  },

...
// Helper utilities to ensure vectors match the Pinecone index dimension.
function getTargetDimension(indexDimension, sampleVectorLength) {
  const envDim = Number(process.env.PINECONE_INDEX_DIM || 0);
  if (Number.isFinite(envDim) && envDim > 0) return envDim;
  if (Number.isFinite(indexDimension) && indexDimension > 0) return indexDimension;
  if (Number.isFinite(sampleVectorLength) && sampleVectorLength > 0)
    return sampleVectorLength;
  return null;
}

function alignVectorDimension(vector, targetDimension) {
  if (!Array.isArray(vector) || !Number.isFinite(targetDimension) || targetDimension <= 0) {
    return vector;
  }
  if (vector.length === targetDimension) return vector;
  // If embedding is larger than index dimension, truncate.
  if (vector.length > targetDimension) {
    return vector.slice(0, targetDimension);
  }
  // If embedding is smaller than index dimension, pad with zeros.
  const padded = vector.slice();
  while (padded.length < targetDimension) padded.push(0);
  return padded;
}
...
const PineconeDB = {
  ...
  connect: async function () {
    ...
    // Support multiple shapes for the describeIndex response across SDK versions.
    const indexDimension =
      indexDescription?.dimension ??
      indexDescription?.database?.dimension ??
      indexDescription?.status?.dimension ??
      null;
    const status = indexDescription?.status || indexDescription;
  ...
  addDocumentToNamespace: async function (
    ...
    try {
      ...
      if (!!vectorValues && vectorValues.length > 0) {
        const { indexDimension } = await this.connect();
        const targetDimension = getTargetDimension(
          indexDimension,
          vectorValues[0]?.length
        );
        for (const [i, vector] of vectorValues.entries()) {
          const alignedValues = alignVectorDimension(vector, targetDimension);
          const vectorRecord = {
            id: uuidv4(),
  ...
  performSimilaritySearch: async function ({
    ...
    const { indexDimension } = await this.connect();
    const rawQueryVector = await LLMConnector.embedTextInput(input);
    const targetDimension = getTargetDimension(
      indexDimension,
      rawQueryVector?.length
    );
    const queryVector = alignVectorDimension(rawQueryVector, targetDimension);

  ...
  keys: async function () {
    try {
      ...
      if (error.name === 'AbortError' || error.name === 'TypeError' || error.message?.includes('fetch')) {
        console.debug('Backend server not available:', error.message);
        return null;
      }
      console.error('Error fetching setup information:', error);
      return null;
    }
  },
  ...
  fetchLogo: async function () {
    try {
      ...
      if (response.ok && response.status !== 204) {
        const isCustomLogo = response.headers.get("X-Is-Custom-Logo") === "true";
      ...
      // Silently handle connection errors (server not running, network issues, etc.)
      if (error.name === 'AbortError' || error.name === 'TypeError' || error.message?.includes('fetch')) {
        // Don't log connection errors as they're expected when backend is not available
        return { isCustomLogo: false, logoURL: null };
      }
